...
class GhostHunter {
	constructor(_options = {}) {
		...
		['client_id', 'client_secret'].forEach(deprecatedInstanceParam => {
			if (this.instance[deprecatedInstanceParam]) {
				const normalParam = deprecatedInstanceParam
					.replace(/_([a-z])/, (match, letter) => letter.toUpperCase());
				console.warn(`[deprecation] - ${deprecatedInstanceParam} is no longer supported. Please use ${normalParam} instead. See https://git.io/f4dV9`);
				this.instance[normalParam] = this.instance[deprecatedInstanceParam];
				this.instance[deprecatedInstanceParam] = undefined;
			}
		});

		['clientID', 'clientSecret', 'url'].forEach(requiredInstanceParam => {
